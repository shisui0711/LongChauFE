...
;// CONCATENATED MODULE: ./src/ts/components/Carousel.ts
function CarouselHandler() {
    for (const prev of $(".mycarousel-prev")) {
        ...
        $(`${target} .mycarousel-prev`).on("click", function () {
            var _a;
            let carouselTarget = $(prev).attr("mycarousel-target");
            let itemsPerPage = Number($(carouselTarget).attr("display-item"));
            let totalItems = $(carouselTarget.concat(" .mycarousel-item")).length;
            let totalPages = Math.ceil(totalItems / itemsPerPage);
            let maxWidth = $(carouselTarget).width();
            let currentPage = Number((_a = $(carouselTarget).attr("current-page")) !== null && _a !== void 0 ? _a : "0");
            if (currentPage > 0) {
    ...
    for (const next of $(".mycarousel-next")) {
        ...
        $(`${target} .mycarousel-next`).on("click", function () {
            var _a;
            let carouselTarget = $(next).attr("mycarousel-target");
            let itemsPerPage = Number($(carouselTarget).attr("display-item"));
            let totalItems = $(carouselTarget.concat(" .mycarousel-item")).length;
            let totalPages = Math.ceil(totalItems / itemsPerPage);
            let maxWidth = $(carouselTarget).width();
    ...
    for (const btnIndicator of $(".mycarousel-indicators [mycarousel-target]")) {
        ...
        $(`${target} .mycarousel-indicators [mycarousel-target-page = ${targetPage}]`).on("click", function () {
            let carouselTarget = $(btnIndicator).attr("mycarousel-target");
            let itemsPerPage = Number($(carouselTarget).attr("display-item"));
            let totalItems = $(carouselTarget.concat(" .mycarousel-item")).length;
            let totalPages = Math.ceil(totalItems / itemsPerPage);
            let targetPage = Number($(btnIndicator).attr("mycarousel-target-page"));
            ...
            let maxWidth = $(carouselTarget).width();
            $(carouselTarget.concat(" .mycarousel-wrapper")).css("transform", `translateX(-${maxWidth * targetPage}px)`);
        });
    }
}

// EXTERNAL MODULE: ./src/ts/header.ts + 1 modules
...
;// CONCATENATED MODULE: ./src/ts/layoutHelper.ts
...
// EXTERNAL MODULE: ./src/ts/localStorage.ts
var localStorage = __webpack_require__(558);
;// CONCATENATED MODULE: ./src/ts/main.ts




let myLocalStorage = new localStorage/* MyLocalStorage */.R();
$(function () {
    LayoutRender();
    AddProductHandler();
    CarouselHandler();
...
function AddProductHandler() {
    for (const btnProduct of $('.product-buy')) {
        let productTarget = $(btnProduct).attr('product-target');
        if (productTarget === null || productTarget === undefined) {
            continue;
        }
        $(btnProduct).on('click', function () {
            let productName = $(productTarget.concat(' .product-name')).text();
            let productPrice = $(productTarget.concat(' .product-price')).text();
            let productPriceOld = $(productTarget.concat(' .product-price-old')).text();
            let productImage = $(productTarget.concat(' .product-image')).attr('src');
            let productUnit = $(productTarget.concat(' .product-unit-short')).text().slice(2);
            let productLink = $(productTarget.concat(' .product-link')).attr('href');
            let isDiscount = $(productTarget.concat(' .product-discount')).css('visibility') === 'visible';
            let product = {
                id: generateGuid(),
                name: productName,
                img: productImage,
