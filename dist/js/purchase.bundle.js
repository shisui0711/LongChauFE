...
;// CONCATENATED MODULE: ./src/ts/components/radios.ts
...
;// CONCATENATED MODULE: ./src/ts/components/switches.ts
function StateChangeHandler() {
    $('.btn-switch').on('click', () => {
        let raw_state = $('.btn-switch').attr('check-state');
        if (raw_state === "true") {
...
;// CONCATENATED MODULE: ./src/ts/purchase.ts
...
function LoadProduct() {
    ...
    cartObject.products.forEach(product => {
        if (!product.selected)
            return;
        let template = `<div id="${product.id}" class="row align-items-center mb-3">
